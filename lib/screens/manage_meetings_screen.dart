...
// Guests Screen - Full screen for viewing guests
class _GuestsScreen extends StatefulWidget {
  final List<Map<String, dynamic>> guests;
  final String userId;
  final VoidCallback onGuestsUpdated;
  
  const _GuestsScreen({
    required this.guests,
    required this.userId,
    required this.onGuestsUpdated,
  });

  @override
  State<_GuestsScreen> createState() => _GuestsScreenState();
}

class _GuestsScreenState extends State<_GuestsScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
      ...
      if (_displayedCount < widget.guests.length) {
        setState(() {
          _displayedCount = (_displayedCount + 20).clamp(0, widget.guests.length);
        });
      }
    }
  }
  ...
  List<Map<String, dynamic>> get _displayedGuests {
    return widget.guests.take(_displayedCount).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // Header with title, delete button, and close button
            Container(
              width: double.infinity,
                        ...
                        child: ConstrainedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // Loading indicator if more guests available
                              if (_displayedCount < widget.guests.length)
                                const Padding(
